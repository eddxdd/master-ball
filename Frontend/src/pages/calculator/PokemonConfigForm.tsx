...
export function PokemonConfigForm({
  ...
  const isAttacker = title === "Attacker";
  const RoleIcon = isAttacker ? Swords : Shield;

  return (
    <div
      id={`calculator-${title.toLowerCase()}`}
      className="flex flex-col gap-3 overflow-hidden rounded-lg border border-border"
    >
      <div
        className="h-1.5 w-full"
        style={{
          backgroundImage: isAttacker ? "var(--gradient-brand)" : "var(--gradient-accent)",
        }}
      />
      <div className="flex flex-col gap-3 p-4 pt-1">
        <h2 className="flex items-center gap-2 font-semibold">
          <RoleIcon
            className={isAttacker ? "size-4 text-primary" : "size-4 text-accent-foreground"}
          />
          {title}
        </h2>
