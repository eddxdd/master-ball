...
The post-loss note field on this same page also runs a real, small ML model **entirely in the browser** — see [`tech-stack.md`](../tech-stack.md#on-deviceedge-ai-optional-stretch--see-roadmapmd)'s "On-device/edge AI" section and [`roadmap.md`](../roadmap.md#phase-7--premium-features-and-stretch-goals)'s Phase 7 item 2: an instant mood/tilt signal on a note, with zero network round-trip after the model's one-time download.
