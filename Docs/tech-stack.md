...
> **How to read this doc:** everything here is optimized for two things simultaneously — (1) it should actually be the right tool for a Pokémon AI coach, and (2) it should be a skill that's genuinely in demand in the Canada/US market right now, not a skill that was in demand two years ago. Where those two things conflicted, we picked the one that's still defensible technically and explained the trade-off rather than picking blind. All specific stats below are as of live 2026 job-market research; treat exact percentages as directional, not gospel.

> **Revision note (cloud/model):** the first pass of this doc leaned on Google Cloud and Gemini more than the aggregate market data actually supports — largely because a Google job posting was the concrete example used to kick off research, which skewed the framing even though the underlying searches were broader. A second, deliberately vendor-agnostic research pass (aggregating multiple independent 2026 "AI Engineer" job-posting analyses covering thousands of listings, plus enterprise LLM-adoption surveys) corrected two decisions: **cloud primary moved from GCP to AWS**, and **LLM provider primary moved from Gemini to a Claude/OpenAI pairing with Gemini as a third, specialist provider**. Both corrections are laid out with numbers below.
...
| Backend | (no task queue) | + **Arq (or Celery) + Valkey** for background jobs (was "Redis" in the first draft — see below) | Replay parsing / meta scraping are async workloads; "shipping to production" is the most repeated theme in 2026 AI hiring research |
...
| **Cloud** | GCP-only | **AWS as primary**, GCP/Vertex AI kept as a documented stretch goal | Corrected after a vendor-agnostic pass — see below. AWS is the clear #1 cloud named specifically in "AI Engineer" job postings, not just cloud postings generally, across every independent study checked |
...
| On-device/edge AI | (not specified) | + **WebGPU + Transformers.js/WebLLM** as an explicit optional stretch item | On-device/edge inference is a genuinely hot, distinct 2026 AI-hiring category (quantization, local inference runtimes) — but it's reachable entirely from the browser, so it doesn't require a native app to demonstrate; see "On-device/edge AI" below |

Everything else in the original draft (React/TS/Vite/Tailwind/TanStack Query/Zustand, FastAPI, PostgreSQL, SQLAlchemy/SQLModel, pgvector-first, WebSockets, Docker, GitHub Actions, scikit-learn → XGBoost → PyTorch progression) was already well-aligned with current hiring data and is kept as-is. The one exception found on re-verification is the caching layer (Redis → Valkey, same role, different underlying engine — see the Cache/queue engine row above and the Backend section below). See inline notes below for the reasoning on each.
...
**Cloud — aggregated across four independent 2026 studies of "AI Engineer" job postings specifically (not general cloud/DevOps postings, which skew even more toward AWS):**

| Study (sample size) | AWS | Azure | GCP |
|---|---|---|---|
| InterviewStack.io analysis (3,449 AI Engineer postings) | 37% | 33% | 25% |
...
AWS is #1 in every single one of these, by a real margin, not a rounding error. GCP is consistently third. The one place GCP has genuine, defensible strength is ML/data-specialist niches specifically (Vertex AI, BigQuery) and it does pay a premium there — but "most requested, most well-rounded" is unambiguously AWS. **Cloud primary is now AWS** (ECS/Fargate or App Runner for hosting, RDS for PostgreSQL+pgvector, S3 for blobs, Bedrock for unified multi-model access — Bedrock natively hosts Claude, several open-weight models, and increasingly others). GCP/Vertex AI moves to the roadmap as a documented stretch goal (see [`roadmap.md`](./roadmap.md)) — useful specifically because it still lets you speak concretely to Vertex AI if a Google-shaped role like the one you linked comes up, just not as the foundation everything else is built on.

*(Re-verified 2026, one nuance worth naming honestly, not hiding:* general cloud infrastructure market share (not AI-engineer-job-posting share specifically) shows AWS still #1 at ~30% but the *slowest-growing* of the big three (+19% YoY), while Azure (~25%, +40% YoY) and GCP (~13%, +63% YoY) are gaining ground fast, with AI workloads cited as the main driver of that growth-rate gap. This doesn't change the recommendation — the decision here was deliberately anchored on *AI-engineer-specific* job postings, not overall infrastructure spend, and AWS still leads that specific metric clearly — but it's worth knowing the broader trend line exists so it doesn't come as a surprise later.)*
...
- **OpenAI** — co-primary, used for the router/classification step and any high-volume simple task, and kept as a first-class provider (not an afterthought) specifically because it's still the single most-adopted API, the one most frameworks/examples/job postings assume by default, and worth being fluent in regardless of which one is "technically better" for this project.
...
This is a genuine multi-vendor architecture with a real provider-abstraction layer (LangChain's chat-model interface makes this closer to a config change than a rewrite), which is itself worth being able to speak to in an interview — it's exactly the pattern the enterprise data says real companies converged on.
...
| Tool | Role |
|---|---|
| **React** | UI library. Still the default hire-for skill; no reason to deviate. |
...
| **shadcn/ui** *(added)* | Accessible, unstyled-then-Tailwind-styled component primitives (Radix under the hood). Shows up repeatedly in current AI-product job postings alongside Tailwind because it's the fastest path to a clean, non-generic-looking UI without a heavy design-system dependency. |
...
No other changes from the original draft here — it was already a strong, modern, hire-relevant frontend stack.
...
**Pass 2:** stepping back and weighing this against what the project is actually for — an AI-engineering portfolio piece, with any real product traction as a bonus, not the goal — changed the conclusion. Two things drove the reversal:
1. **None of it teaches or demonstrates an AI skill.** Capacitor setup, Android signing, Play Console policy compliance, and ongoing release/review cycles are real, recurring, non-AI engineering overhead. A portfolio piece for AI Agent Developer roles is judged on the agent/RAG/eval/MCP work, not on app-store packaging.
2. **Specifically researched "should the AI angle change this," and it didn't turn out to favor native.** There are two genuinely distinct "AI on phones" trends: (a) AI-powered app-store discovery (Google's "Ask Play" conversational search, announced at I/O 2026) — irrelevant to a build decision, it's about how *other* apps get found; and (b) on-device/edge AI inference, which is a real, hot, distinct 2026 hiring category — but it's reachable entirely from a browser via WebGPU (see "On-device/edge AI" below), so it doesn't argue for a native app either. The one genuinely native-locked capability, Android's Gemini Nano via the system-level AICore service, [Source](https://developer.android.com/ai/gemini-nano) is real but narrow (Android-only, one vendor) — kept as an optional future plugin idea, not a reason to commit to the app-store path now.
...
**Why this earns a place on the roadmap even though it's not core:** on-device/edge inference is a real, distinct, currently-hot AI-hiring category (dedicated roles like ML Mobile Engineer / Edge Inference Engineer exist specifically for this, built around quantization and local inference runtimes) — [Source](https://mobile.wednesday.is/writing/staffing-on-device-ai-enterprise-mobile-team-roles-org-2026) but it's a different skill from the cloud-agent-orchestration work that's the rest of this stack, and it's genuinely demonstrable from the browser, with no native app required. A concrete, defensible use in this product: running a small local model against a post-loss note for an instant mood/tilt signal, without a network round-trip, as part of the Mental-Game Coach — good interview material specifically because it's a real "why on-device here and not elsewhere" design decision, not a tacked-on demo.
...
| **FastAPI** | Web framework. Native Pydantic integration (validation *and* structured LLM outputs use the same models), async-first (matters for streaming + concurrent tool calls), and it's the framework named in ~17% of LangChain-ecosystem job postings — the highest of any web framework named. |
...
**Decision note (corrected from the first draft — see "Cloud and model-provider decision, revisited" above for the full numbers):** AWS is now primary. It's the clear #1 cloud named specifically in AI Engineer job postings (37–44% across four independent 2026 studies, vs. GCP's 23–29%), and "AWS + RAG" has the highest co-occurrence lift of any cloud/skill pairing checked — Bedrock + a Postgres/pgvector RDS instance + S3 + a container runtime is a genuinely common, well-supported production RAG stack, not just a popular resume line. Because everything is Docker-first, a documented stretch goal (see [`roadmap.md`](./roadmap.md)) is to redeploy the same containers to **Google Cloud Run with Vertex AI** — worth doing specifically so you can speak concretely to Vertex AI experience if a GCP-shaped role (like the one that originally anchored this research) comes up, without it being the foundation the whole project depends on.
...
| **Langfuse** *(added — learn both, run one)* | Open-source, self-hostable, vendor-neutral alternative. Worth actually standing up (even just locally) specifically to be able to compare it against LangSmith firsthand. LangSmith and Langfuse together are ~56% of all eval-tool mentions in 2026 job postings — knowing both, and articulating the hosted-vs-self-hosted trade-off, covers more of the market than mastering either one alone. *(Re-verified 2026: Langfuse was acquired by ClickHouse in January 2026 — capabilities are unchanged and it's still the most-recommended self-hosted/open-source option across every source checked, but worth knowing about if it comes up.)* |
...
Kept exactly as proposed — this progression (simple → tabular boosting → deep learning only when justified) matches how "ML engineering" is actually described in current job postings: shipping and operating models, not chasing model complexity for its own sake.
...
| **Kubernetes (EKS)** | Only if/when ECS/Fargate's simpler model genuinely becomes limiting (e.g., needing custom autoscaling policies per-service, or a service mesh). | Stays "later" — Fargate already gives most of the container-orchestration benefit without the operational overhead, and that's a defensible reason to *not* reach for k8s prematurely, which is itself worth being able to say in an interview. |
