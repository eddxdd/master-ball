...
A small XGBoost model, served **alongside** the LLM-based team analysis (not instead of it) — see [`roadmap.md`](../roadmap.md#phase-7--premium-features-and-stretch-goals)'s item 5. This is a structured, numeric, low-latency prediction over engineered tabular features — exactly the kind of task classical gradient-boosted trees are the industry-standard tool for, and an LLM call would be slower, costlier, and no more accurate at.
